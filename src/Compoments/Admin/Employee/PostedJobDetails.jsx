...
function PostedJobDetails(){
    ...
    const disableJob = () => {
        setLoading(true);
        axios.put(`${config.api.baseURL}${config.api.admin.disableJob}${job.jobId}`,{},{
            headers: {
    ...
    return (
        ...
        <div className='flex-grow'>
          ...
          {loading ? ( <div className="flex flex-col justify-center items-center h-screen">
                ...
                <h1 className="font-bold text-xl underline my-1">{job.jobTitle}</h1>
                <h1 className="font-bold text-sm my-1">{job.companyName}</h1>
                <h1 className="my-1 ">{job.jobLocation}</h1>
                <h1 className="my-1">Experience: {job.experienceRequired}</h1>
                <h1 className="my-1">Posted On: {job.postedOn}</h1>
                <h1 className="mt-2 font-bold">Know more about the job</h1>
                <a className="my-1 text-blue-500 underline" target="_blank" href={job.jobDescriptionLink}>{job.jobDescriptionLink}</a>

                <h1 className="my-2 font-bold">Job Posted by</h1>
                <h1 className="my-1">Employee Name: {job.jobPosterName}</h1>
                <h1 className="my-1">Company Name: {job.companyName}</h1>
                <button  onClick={disableJob}className="bg-gray-700 text-white p-2 rounded my-2 mx-10">{job.enabled?' Disable Job': 'Enable Job'}</button>
                </div>
